from __future__ import annotations

import argparse
from datetime import datetime
from pathlib import Path

import yaml

from hsrl_sim.experiments import run_optical_smoke


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path, default=None)
    args = parser.parse_args()
    config = yaml.safe_load(args.config.read_text(encoding="utf-8"))
    output_dir = args.output_dir or Path("outputs") / datetime.now().strftime("optical-%Y%m%d-%H%M%S")
    result = run_optical_smoke(
        output_dir=output_dir,
        seed=int(config["seed"]),
        n_truth=int(config["n_truth"]),
        replicates=int(config["replicates"]),
        alpha_errors=tuple(float(value) for value in config["alpha_errors"]),
        multistart=int(config["multistart"]),
        radius_points=int(config["radius_points"]),
        radius_min_um=float(config.get("radius_min_um", 0.01)),
        radius_max_um=float(config.get("radius_max_um", 15.0)),
        coverage_target=float(config.get("coverage_target", 0.90)),
        max_interval_factor=float(config.get("max_interval_factor", 10.0)),
        common_correlation=float(config.get("common_correlation", 0.10)),
        same_wavelength_correlation=float(config.get("same_wavelength_correlation", 0.35)),
        model_mismatch=bool(config.get("model_mismatch", False)),
    )
    print(f"output_dir={output_dir}")
    for row in result.summary_records:
        if row["scenario_id"] != "all":
            continue
        print(row)


if __name__ == "__main__":
    main()
